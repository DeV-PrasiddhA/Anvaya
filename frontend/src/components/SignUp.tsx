...
import { registerUserInSupabase } from '../api';
import { signInWithGoogle, signInWithEmail, signUpWithEmail } from '../supabaseClient';
...
export default function SignUp({ onNavigateBack, onNavigateToDashboard }: SignUpProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setAuthLoading(true);
      // If email and password are provided, register with Supabase Auth
      if (email && password) {
        try {
          await signUpWithEmail(email, password, formData.name);
        } catch (authErr: any) {
        ...
        {phase === 'fill-form' && currentRoleConfig && (
          <div className="w-full max-w-lg p-6 md:p-8 rounded-2xl glass-panel border border-white/20 bg-background/90 shadow-2xl relative">
            ...
            <button
              ...
              className="w-full mb-4 py-3 px-4 rounded-xl border border-outline-variant/60 bg-white hover:bg-slate-50 text-slate-800 text-sm font-semibold flex items-center justify-center gap-3 transition-all shadow-sm cursor-pointer"
            >
            ...
            <div className="flex items-center gap-3 my-3">
              ...
              <span className="text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">Or with Email / Gmail</span>
              <div className="h-px bg-outline-variant/40 flex-1"></div>
            </div>
